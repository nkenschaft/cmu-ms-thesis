...
\begin{itemize}
    ...
    \item Given some matrix $A \in \R^{n \times m}$, I write $A[r_1:r_2,c_1:c_2]$ to do indicate the matrix of entries taken from rows $r_1$ through $r_2$ (inclusive) and columns $c_1$ through $c_2$ (inclusive) of $A$.
        If any value is left empty, I mean to include all the way to the first/last row/column depending on which entry is empty (e.g. $A[:,:2]$ is the first two columns of $A$).
        For any programmers, this is just the Python library \texttt{numpy}'s syntax.
\end{itemize}