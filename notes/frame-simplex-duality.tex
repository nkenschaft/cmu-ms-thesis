...
\begin{definition}
    ...
    \[
        \card{\mu_\bullet} := \sup_{x \in X} \min \set{n \in \N: \exists g_1,...,g_n \in g, \alpha_1,...,\alpha_n \in \R, \mu_x = \sum_{i \leq n} \alpha_i \delta_{x_i} \tand \sum_{i \leq n} \alpha_i = 1}.
    \]
...
\begin{proposition}
    ...
    \begin{proof}
        ...
        But observe that we must have $\card{G_x} < k$ by the definition of the $G$-action on $\Delta^G$, so we immediately have the nontrivial subgroup that we desire.
