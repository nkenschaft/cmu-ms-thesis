...
\begin{theorem}\label{thm:weight-frame-equivalence}
    ...
    \begin{proof}
        ...
        \footnote{This construction was given by Claude via \href{https://claude.ai/share/1bbec844-7ccc-452b-86cb-46641341f6ea}{this chat}.}
        First, observe that the quotient map $q : Gx \to G/G_x$ is a homeomorphism.
        Then by Kuratowski-Ryll-Nardzewski Measurable Selection Theorem % TODO: check this
        there exists a measurable (i.e. Borel) section $s : G/G_x \to Gx$ of $q$.
        Letting $\Hat{s_x} : G/G_x \times G_x \to G$ be the map $\Hat{s}(g,h) = s(g)h$ and $m_x$ be the Haar measure on $G_x$, we now define $\mu_x = \Hat{s}_* ((q_* \nu_x) \otimes m_x)$, i.e. for any Borel function $f : G \to \R$ we have.
        \[
            \int_G f d\mu_x = \int_{G/G_x} \int_{G_x} f(s(q(g)) \cdot h) d m_x (h) d \nu_x (g).
        \]
        Note that $\mu_x$ is independent of the choice of $s$. % TODO: check this
        To see that $\mu_\bullet$ is equivariant, let $g \in G$ and $x \in X$ and observe
        \[
            \mu_{gx}
            = 
        \]
