...
\begin{proposition}\label{prop:SOd-secat-2}
    ...
    \begin{proof}
        ...
        By proposition \ref{prop:psd-iff-transpose-product} we know that $\R^{d \times d}_*/O(d)$ is homeomorphic to the (disjoint) union of full rank matrices in $\S^d_+$ and $\S^d_-$ (here denoted $S_+$ and $S_-$) via the map $q : \R^{d \times d}_* \to S_+ \sqcup S_-$ defined as follows:
        \[
            q(A) = 
            \begin{cases}
