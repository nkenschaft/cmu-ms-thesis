...
\begin{proposition}\label{prop:SOd-secat-1}
    ...
    \begin{proof}
        ...
        By lemma \ref{lem:Od-cong-psd} we know that $\R^{d \times d}_*/\SO(d)$ is homeomorphic to the (disjoint) union of full rank matrices in $\S^d_+$ and $\S^d_-$ (here denoted $S_+$ and $S_-$) via the map $q : \R^{d \times d}_* \to S_+ \sqcup S_-$ defined as follows:
        \[
            q(A) = 
            \begin{cases}
        ...
        Next, we define $s : \R^{d \times d}_*/\SO(d) : \to \R^{d \times d}_*$ by
        \[
            s(A) = 
            \begin{cases}
                A^{\f12}        &\tif A \in S_+ \\
            ...
            \end{cases}
        \]
        where $M^{\f12}$ denotes the matrix square root where $F$ is some orthogonal transformation that flips the sign of its input's determinant, i.e. $F \in O(d) \sminus SO(d)$.
        Observe that $s$ is well-defined and continuous since it is continuous on $S_+$ and $S_-$ which are disjoint open sets.
        ...
        \[
            ...
            = A.
        \]
        Therefore $s$ is a section of $q$, so we conclude that $\ssecat(q) = 1$.
    \end{proof}
\end{proposition}
...
% TODO: clean these up
\begin{theorem}
    Let $q : \R^{d \times n}_* \to \R^{d \times n}_* / O(d)$ be the quotient map.
    Then $\secat(q) < \infty$.
    \begin{proof}
        First consider $p : \R^{d \times n}_*/\Rp \to (\R^{d \times n}_*/O(d))/\Rp \cong \Gr_k(\R^n)$
        Recall from before that $p$ is a principle $O(d)$-bundle, so for all $x \in \Gr_k(\R^n)$ we can find a local section $s_x : U_x \to \R^{d \times n}_*/\Rp$ of $p$ where $U_x \ni x$ is open.
        % TODO: cite source for compactness
        Then by compactness of $\Gr_k(\R^n)$, there exists some finite collection $\set{x_i}_{1 \leq i \leq m} \sub \Gr_k(\R^n)$ such that $\Gr_k(\R^n) = \cup_i U_{x_i}$.
        To conclude, observe that $\R^{d \times n}_*/O(d) = \cup_{i} \Rp U_{x_i}$ and we have local sections $r_i : \Rp U_i \to \R^{d \times n}_*$  given by $r_i(tx) = ts_i([x])$ where $\norm{x} = 1$.
        Therefore by definition we have that $\secat(q) \leq m < \infty$.
    \end{proof}
\end{theorem}

\begin{theorem}
    Let $q : \R^{d \times n}_* \to \R^{d \times n}_* / SO(d)$ be the quotient map.
    Then $\secat(q) < \infty$.
    \begin{proof}
        This follows exactly the same way as the previous theorem.
    \end{proof}
\end{theorem}


We can actually provide much sharper bounds than this.


\begin{theorem}\label{thm:Od-full-rank-secat-upper-bound}
    Let $q : \R^{d \times n}_* \to \R^{d \times n}_* / \O(d)$ be the quotient map where $d \leq n$.
    Then $\secat(q) \leq \binom{n}{d}$.
    
    \begin{proof}
        \footnote{The idea for this construction was given by Claude.ai in \href{https://claude.ai/share/48cf83a5-596b-4b34-a528-11fade173839}{this conversation}.}
        First, define $N^n_d := \set{\alpha \in [n]^d : \alpha_1 < \cdots < \alpha_d}$.
        Observe that $N^n_d$ is just the collection of subsets of $[n]$ of size $d$, hence $\card{N^n_d} = \binom{n}{d}$.
        Then for each $\alpha \in N^n_d$, we define $V_\alpha := \set{A \in \R^{d \times n}_* : \det (A_\alpha) \neq 0}$ where $A_\alpha$ denotes the matrix with columns from $A$ specified by the indices of $\alpha$ (i.e. $A_\alpha = [A_{\alpha_1} \cdots A_{\alpha_d}]$).
        Observe that each $V_\alpha$ is indeed open with $\R^{d \times n}_* = \cup_{\alpha \in N^n_d} V_\alpha$.
        Furthermore, $V_\alpha$ is $\O(d)$-invariant since for any $Q \in O(d)$ and $A \in V_\alpha$, we have $\det((QA)_\alpha) = \det(QA_\alpha) = \det(Q)\det(A_\alpha) \neq 0$.
        This means we also have open sets $U_\alpha = q[V_\alpha] \sub \R^{d \times n}_* / \O(d)$ with $U_\alpha = \inv{q}[V_\alpha]$ and $\R^{d \times n}_* = \cup_{\alpha \in N^n_d} U_\alpha$.
        
        We now define $r_\alpha : V_\alpha \to \R^{d \times n}_*$ for each $\alpha \in N^n_d$ by $r_\alpha(A) := \bigp{(A_\alpha^T A_\alpha)^{\f{-1}{2}}}^T A_\alpha^T A$.
        Note that $A_\alpha (A_\alpha^T A_\alpha)^{\f{-1}{2}} \in O(d)$ (see \cite{wikipedia:polar-decomposition}), so we also have
        \[
            \pinv{A_\alpha (A_\alpha^T A_\alpha)^{\f{-1}{2}}}
            = \bigp{A_\alpha (A_\alpha^T A_\alpha)^{\f{-1}{2}}}^T
            = \bigp{(A_\alpha^T A_\alpha)^{\f{-1}{2}}}^T A_\alpha^T \in O(d)
        \]
        and hence $r_\alpha(A) \in O(d)A$.
        Additionally, for any $Q \in O(d)$ we have
        \begin{align*}
            r_\alpha (QA)
            &= \bigp{((QA)_\alpha^T (QA)_\alpha)^{\f{-1}{2}}}^T (QA)_\alpha^T QA \\
            &= \bigp{((QA_\alpha)^T Q A_\alpha)^{\f{-1}{2}}}^T (QA_\alpha)^T QA \\
            &= \bigp{(A_\alpha^T Q^T Q A_\alpha)^{\f{-1}{2}}}^T A_\alpha^T Q^T QA \\
            &= \bigp{(A_\alpha^T A_\alpha)^{\f{-1}{2}}}^T A_\alpha^T A \\
            &= r_\alpha(A)
        \end{align*}
        so $r_\alpha$ is $O(d)$-invariant.
        Putting these two facts together, we have that each $r_\alpha$ descends to a local section $s_\alpha : U_\alpha \to \R^{d \times n}_*$ of $q$.
        Since there are $\binom{n}{d}$-many sets $U_\alpha$ with sections $s_\alpha$, we conclude that $\secat(q) \leq \binom{n}{d}$.
    \end{proof}
\end{theorem}


\begin{theorem}\label{thm:SOd-full-rank-secat-upper-bound}
    Let $q : \R^{d \times n}_* \to \R^{d \times n}_* / \SO(d)$ be the quotient map where $d \leq n$.
    Then $\secat(q) \leq \binom{n}{d}$.
    \begin{proof}
        This follows from a combination of the arguments in proposition \ref{prop:SOd-secat-1} and \ref{thm:Od-full-rank-secat-upper-bound}.
    \end{proof}
\end{theorem}
...
% TODO: find a piecewise canonicalization

I think it may actually be impossible to find \emph{any} collection of local sections for the entire space $\R^{d \times n}$.
If we have $A \in \R^{d \times n}$ with $\rank(A) \leq d-1$ (maybe $\rank(A) \leq d-2$?) any open set $U \sub \R^{d \times n}/O(d)$ containing $O(d)A$ will also contain $O(d)L$ for some $L \in \R^{d \times n}_*$ by density, and this seems like it should cause problems.
% TODO: flesh this out


\begin{theorem}\label{thm:no-local-Od-sections}
    Let $q : \R^{d \times n} \to \R^{d \times n}/\SO(d)$ be the quotient map for $n > d$.
    Then $\ssecat(q) = \infty$.
    \begin{proof}
        \Afsoc that $\ssecat(q) < \infty$, which in particular there is some open set $U \sub \R^{d \times n}/O(d) \cong \S^n_{\leq d}$ with a local section $s : U \to \R^{d \times n}$ of $q$..
        Then with any norm $\norm{\cdot}$ on $\S^n$, we must have $r > 0$ such that $B(0,r) \cap \S^n_d \sub U$.
        Additionally, note that $r \S^n_d = \S^n_d$, so without loss of generality we may take $U = B(0,1) \cap \S^n_d$.
        Define $\lambda : \S^n_d \to \R$ by
        \[
            \lambda(P)
            :=
            \begin{cases}
            ...
            \end{cases}
        \]
        which is continuous, and consider the map $\Hat{s} : \S^n_d \to \R^{d \times n}$ given by
        \[
            \Hat{s}(P)
            := \f{1}{\sqrt{\lambda(P)}} s\bigp{\lambda(P) P}.
            := \f{1}{\sqrt{\lambda(P)}} s\bigp{\lambda(P) P}.
        \]
        ...
        Now let $P \in \S^n_d$ and recall that there must be some $A \in \R^{d \times n}$ such that $A^T A = P$.
        We then have that 
        \begin{align*}
            \Hat{s}(P)
            &= \f{1}{\sqrt{\lambda(P)}} s\bigp{\lambda(P) A^T A} \\
            &= \f{1}{\sqrt{\lambda(P)}} s\bigp{\bigp{\sqrt{\lambda(P)} A}^T \bigp{\sqrt{\lambda(P)} A}} \\
            &= \f{1}{\sqrt{\lambda(P)}} Q \bigp{\sqrt{\lambda(P)} A} \tag{for some $Q \in O(d)$} \\
            &= QA
        \end{align*}
        and so $q(\Hat{s})(P) = P$.
        This means that $\hat{S}$ is a global section of $q$, but we saw in \ref{thm:no-Od-canon-for-dxn} that this is impossible.
        We conclude that there can be no local section of $q$ at $0$, and hence $\ssecat(q) = \infty$.
    \end{proof}
\end{theorem}

\begin{theorem}\label{thm:no-local-SOd-sections}
    Let $q : \R^{d \times n} \to \R^{d \times n}/\SO(d)$ be the quotient map for $n \geq d$.
    Then $\ssecat(q) = \infty$.
    \begin{proof}
        The proof follows exactly the same as theorem \ref{thm:no-local-Od-sections}, except replacing theorem \ref{thm:no-Od-canon-for-dxn} with theorem \ref{thm:no-SOd-canon-for-dxn}.
    \end{proof}
\end{theorem}
