...
\begin{theorem}\label{thm:no-local-Od-sections}
    ...
    Then $\ssecat(q) = \infty$.
    \begin{proof}
        \Afsoc that $\ssecat(q) < \infty$, which in particular there is some open set $U \sub \R^{d \times n}/O(d) \cong \S^n_{\leq d}$ such that $U \ni 0$ with a local section $s : U \to \R^{d \times n}$ of $q$.
