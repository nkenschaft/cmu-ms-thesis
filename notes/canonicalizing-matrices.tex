...
\begin{theorem}\label{thm:no-Od-canon-for-dxn}
    If $n > d \geq 4$, $\R^{d \times n}$ has no canonicalization with respect to $\O(d)$.
    \begin{proof}
        ...
        Next, observe that $S^{d(d+1) - 1}/O(d)$ is homeomorphic to $\set{A \in \S^{d+1} : \det(A) = 0}/\Rp$ by corollary \ref{cor:Rnd/Od-cong-Snd}, which is in turn homeomorphic to the boundary of the ball of dimension $(d+1)(d+2)/2 - 1$, i.e. the sphere of one dimension less than that, by proposition \ref{prop:PSD-mod-scale-cong-ball}.
        Then a contradiction once again arises by considering the induced maps on homology.
        We require $d \geq 4$ so that $d^2 - 1 > (d+1)(d+2)/2 - 2$ and this last step actually produces a contradiction.
