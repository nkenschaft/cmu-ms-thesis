...
\begin{remark}
    ...
    This is captured by the action of $O(k)$ on $S_k(n)$ by right multiplication, hence $G_k(n) \cong V_k(n) / O(k)$.
\end{remark}
...
\begin{remark}
    There is a one-to-one correspondence between $\R^{d \times n}_*$ and $V_d(n)$ via $A \mapsto A^T$.
\end{remark}
