...
\begin{proposition}
    ...
    \[
        \cupln(\Gr(n,3))
        = \begin{cases}
            2^{s + 2} - 3 \cdot 2^{p - 1} - 4 &\text{ if } n = 2^{s+1} - 2^{p} \text{ for some } p \geq 1 \\
            2^{s + 2} - 3 \cdot 2^{p - 1} - 2 + t &\text{ if } n = 2^{s+1} - 2^{p} + 2 + t\\
            &\text{ for some } p \geq 1, 0 \leq t \leq 2^{p-1}-2 \\
...
\begin{proposition}
    Given a positive integer $s$, we have
    \[
        \cupln(\Gr(2^s+1,4)) = 2^{s+1}+2^s-7.
    \]
    Furthermore, given integers $0 \leq r < s$ and $0 \leq t < 2^r - 1$, we have
    \[
        \cupln(\Gr(2^s+2^r+t+1,4)) = 2^{s+1} + 2^s + 2^{r+1} + t - 7.
    \]
\end{proposition}

% TODO: this proposition causes the 2 above not to work for n \leq 9. not sure why.
\begin{proposition}
