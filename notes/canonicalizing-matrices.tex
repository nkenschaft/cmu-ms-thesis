...
\begin{lemma}
    With $O(d)$ acting on $\R^{d \times n}$ via matrix multiplication,
    $\R^{d \times n}_{free} = \R^{d \times n}_*$.
    \begin{proof}
...
\begin{definition}
    ...
    The \emph{Stiefel manifold} $V_k(n)$ is the set of orthonormal basis matrices for $k$-dimensional subspaces of $\R^n$, i.e.
    \[
        V_k(n) := \set{U \in \R^{n \times k} : U^T U = I}
    \]
\end{definition}

\begin{definition}
    (From \cite{bendokat2024grassmanhandbook})
    The \emph{Grassmannian manifold}
    \[
        G_{d,n} := V_{d,n}/O(d)
    \]
    is the set of $d$-dimensional subspaces of $\R^n$.
\end{definition}
