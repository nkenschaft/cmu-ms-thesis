...
\begin{remark}
    There is a natural projection $\Tilde{\pi} : \St(n,k) \to \Tilde{\Gr(n,k)}$ where $\pi(B)$ is the plane $\Span B$ with orientation given by the order of the basis $B$.
\end{remark}

\begin{proposition}
    Let $\pi^*(\Tilde{E}(n,k))$ be the pullback of the vector bundle $p : \Tilde{E}(n,k) \to \Gr(n,k)$ via the projection $\Tilde{pi} : \St(n,k) \to \Tilde{\Gr}(n,k)$.
    Then the induced map $\pi^* : H^*(\Gr(n,k)) \to H^*(\St(n,k))$ is zero on the Stiefel-Whitney classes $w_i(\Tilde{E}(n,k))$.
    \begin{proof}
