...
\begin{theorem}
    ...
    A weakly equivariant frame $\mu_bullet$ is continuous (i.e. robust) if and only if $\II_\mu$ preserves continuity.
    \begin{proof}
        ...
        To see this, define $f_x : G/G_x \to \R$ by
        \[
            f_x (g G_x) := \int_{G_x} f(gs) ds
        \]
        where the integral is with respect to the left-invariant Haar measure on $G_x$.
        This is well-defined since for $g' = gs'$ with $s' \in G_x$ we have
        \[
            f_x (g' G_x)
            = \int_{G_x} f(g's) ds
            = \int_{G_x} f(gs's) ds
            ...
            = f_x (g G_x)
        \]
        where the third equality follows from the left-invariance of the Haar measure on $G_x$.
        Now let $q_x : G \to G/G_x$ and $p_x : Gx \to G/G_x$ be the quotient and projection maps, and observe that $p_x$ is a homeomorphism.
        Then we compute
        \begin{align*}
            \int_G f d\angles{\mu_{x_n}}_x 
            &= \int_{G_x} \int_G f(g) d s_* \mu_{x_n} ds \\
            &= \int_{G_x} \int_G f(sg) d \mu_{x_n} ds \\
            &= \int_G \int_{G_x} f(sg) ds d \mu_{x_n} \\
            &= \int_G f_x (q_x(g)) d \mu_{x_n} \\
            &= \int_{G/G_x} f_x d (q_x)_* \mu_{x_n} \\
            &= \int_{Gx} (f_x \circ p_x) d (\inv{p_x})_* (q_x)_* \mu_{x_n}
        \end{align*}
        \begin{align*}
            \II_\mu (f) (x_n)
            &\to \II_\mu (f) (x_n) \\
            \int_G f(\inv{g} x) d\mu_{x_n}
            &= \int_{G_x} \int_G f(\inv{g} x) ds_*\mu_{x_n} ds
            \to
            \int_G f(\inv{g} x) d\mu_{x}
        \end{align*}
    \end{proof}
\end{theorem}
